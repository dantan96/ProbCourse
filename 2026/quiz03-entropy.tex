...
The rover's code is optimal for $p$, with $L^*=1.75$.
...
The only profiles with Kraft sum $1$ are $(2,2,2,2)$ and $(1,2,3,3)$.
...
The rover's code (assigning $0$ to the symbol of probability $0.45$, $10$ to the symbol of probability $0.25$) is optimal, $L^*=1.85$, and $H(p)=1.840$.

(e) $p=(0.3,0.3,0.2,0.2)$: the profile $(1,2,3,3)$ gives $L=0.3+0.6+0.6+0.6=2.1>2$.
The fixed-length code is optimal, $L^*=2$, and $H(p)=1.971$.
...
$H(X\mid C)=\tfrac12(0.469+1)=0.734$ bits.
...
Since $C$ is uniform on two values, $H(C)=1$, and $H(C)-H(C\mid X)=1-0.853=0.147=I(X;C)$, as required.
...
Averaging the two pmfs coordinate by coordinate, $\bar P=(\tfrac5{16},\tfrac3{16},\tfrac3{16},\tfrac5{16})$.
...
$H(\bar P)=2\cdot\tfrac5{16}\log\tfrac{16}5+2\cdot\tfrac3{16}\log\tfrac{16}3=1.049+0.906=1.954$ bits per instruction.
...
Ordering the four: known mission $1.75<$ minimal cost $1.954<$ fixed-length $2<$ old code $2.1875$.
...
$I(M;X)=1.954-1.75=0.204$ bits per instruction.
...
The fixed-length code is the best single-instruction code, with cost $2$ bits per instruction.
...
$H(X_{i+1}\mid X_i)=\tfrac34\cdot0.469+\tfrac14\cdot0.881=0.572$ bits per symbol.

(c) $H(\pi)=h(\tfrac14)=0.811$ bits. The difference is $0.811-0.572=0.239$ bits.
This difference is $I(X_i;X_{i+1})$, the mutual information of two consecutive symbols (slide ``Entropy Rate at Most Monographic Entropy'').
...
$H(X_1,X_2,X_3)=H(\pi)+2\cdot0.572=0.811+1.144=1.955$ bits.

(e) The string $abc$ has probability $\pi(a)Q(a,b)Q(b,c)$. For $000,001,010,011,100,101,110,111$ these are
$0.6075,\ 0.0675,\ 0.0225,\ 0.0525,\ 0.0675,\ 0.0075,\ 0.0525,\ 0.1225$
(for example, $000$: $0.75\cdot0.9\cdot0.9=0.6075$). Then $\sum_{abc}\Prob(abc)\log\frac1{\Prob(abc)}=1.955$ bits, agreeing with (d).
...
The waste is $0.811-0.572=0.239$ bits per symbol, the mutual information of (c).
