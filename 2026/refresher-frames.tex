...
\begin{frame}{Indicator Functions}
  \begin{block}{Definition}
    ...
    \]
  \end{block}
  \words{$\ind_A$ records as a number whether $A$ occurs: it equals $1$ when $A$ occurs and $0$ otherwise, so $\Prob(\ind_A=1)=\Prob(A)$.}
  \begin{itemize}
...
\begin{frame}{Distribution Function (cdf)}
  \begin{block}{Definition}
    ...
    \]
  \end{block}
  \words{$F_X(x)$ is the probability that $X$ is at most $x$; it exists for every random variable, discrete or continuous.}
  \begin{itemize}
...
\begin{frame}{Probability Mass Function (pmf)}
  \begin{block}{Definition}
    ...
    \]
  \end{block}
  \words{$p_X$ gives the probability of each possible value, and every probability $\Prob(X\in A)$ is a sum of these.}
  \begin{itemize}
  ...
  \begin{columns}[T]
    \begin{column}{0.62\textwidth}
      ...
      \begin{itemize}
        ...
        \item $\Prob(X=x)=\int_x^x f_X=0$ for every $x$, and $f_X(x)$ may exceed $1$; the uniform distribution on $[0,\tfrac12]$ has $f_X=2$ there.
      \end{itemize}
    \end{column}
...
\begin{frame}{Conditional Density}
  ...
  \begin{block}{Definition}
    ...
    \]
  \end{block}
  \words{The density of $X$ when $Y=y$ is known: the function $x\mapsto f_{X,Y}(x,y)$, divided by $f_Y(y)$ so that it integrates to $1$.}
  \begin{itemize}
    \item The two arguments play different roles: $y$ is the \emph{observed} value, restricted to where $f_Y(y)>0$, and $x$ is the variable of the resulting density.
