...
\begin{frame}{The Bernoulli Process and the Simple Random Walk}
  ...
  \begin{itemize}
    ...
    \item \textbf{Independent increments}: for $m<n$, $S_n-S_m=\xi_{m+1}+\dots+\xi_n$ depends only on tosses after $m$, so it is independent of $(S_0,\dots,S_m)$ and has the same distribution as $S_{n-m}$. After time $m$ the walk behaves as a fresh walk started from $S_m$.
  \end{itemize}
\end{frame}
...
\begin{frame}{First-Step Analysis: Expected Time to Leave $(-a,a)$}
  ...
  \begin{block}{First-step analysis}
    Condition on the first step, as for the waiting time for $\st{HH}$ (Conditional Expectation lecture, Application 2). After one step the walk is at $i\pm1$ and, by independent increments, the rest of the walk is a fresh walk from there:
    \[
...
\begin{frame}{Why the Paths Are Nowhere Differentiable}
  ...
  \end{block}
  \begin{block}{Why}
    By (iii), $\frac{B_{t+h}-B_t}{h}\sim N\big(0,\tfrac1h\big)$: the difference quotient has standard deviation $1/\sqrt h\to\infty$ as $h\to0$, so it cannot converge. The $\Delta_k$ are independent $N(0,\tfrac tn)$ with $\E[\Delta_k^2]=\tfrac tn$ and $\Var(\Delta_k^2)=2(\tfrac tn)^2$; sum over $k$.
  \end{block}
...
\begin{frame}{The Casino's Edge in Numbers, and Why Doubling Fails}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \end{tabular}
      \par\smallskip
      \words{At a $1\%$ disadvantage, doubling $\pounds50$ one pound at a time succeeds about one time in eight; the last three rows are the same goal with stakes of $\pounds5$, $\pounds10$ and $\pounds50$, and with fewer rounds the disadvantage accumulates less.}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Doubling (the ``martingale'' system)}
        Bet $1$; after each loss double the stake; after a win pocket the net $\pounds1$ and restart. With capital $2^k-1$ the gambler can absorb $k-1$ consecutive losses; $k$ losses in a row exhaust the capital.
...
\begin{frame}{The Infinite Target: Three Regimes}
  ...
  \begin{center}
    ...
    \begin{tabular}{@{}lccl@{}}
      ...
      \midrule
      unfavourable, $p<q$ & $1$ & $\dfrac{i}{q-p}<\infty$ & drifts down; reaches $0$ \\
      fair, $p=q$ & $1$ & $\infty$ & no drift; reaches $0$ eventually \\
      favourable, $p>q$ & $(q/p)^i<1$ & $\infty$ (may never be ruined) & drifts up; may never reach $0$ \\
      \bottomrule
    \end{tabular}
  \end{center}
  \begin{itemize}
...
\begin{frame}{Return to the Opening Problem}
  ...
  \begin{itemize}
    ...
    \item The plan ``win $\pounds1$ and stop'' fails one time in ten, and each failure costs $\pounds50$; the mean gain is $0.8995\cdot1-0.1005\cdot50\approx-\pounds4.12$, which equals $(p-q)\,\E[T]=-\tfrac2{38}\cdot78.4$. No staking plan has positive mean gain, since every plan is a sum of bets each of mean $-\tfrac2{38}$ per pound staked.
  \end{itemize}
...
\begin{frame}{PageRank: The Stationary Distribution of a Random Surfer}
  ...
  \end{columns}
  \begin{itemize}
    \item The jump is needed because a page with no out-links, or a group of pages with no exits, traps the surfer, and the chain with matrix $\mathbf H$ alone may have no unique $\pi$ or may fail to converge (slides~\ref{fr:classes}--\ref{fr:convwhy}). With $d<1$ every entry of $\PP$ is positive, so the results below apply.
  \end{itemize}
\end{frame}
...
\begin{frame}{Return to Question 3: Days Until Rain}
  ...
  \begin{itemize}
    \item For the vowel/consonant chain, the expected number of letters until the first vowel is $2$ from $\st C$ (a geometric waiting time) and $1+0.8\cdot2=2.6$ from $\st V$; until the first occurrence of $\st{VV}$ it is $k_{\st C}=15$ and $k_{\st V}=13$.
