...
\begin{frame}{Conditional Expectation}
  \small
  The lecture is organised around one picture, the table of outcomes of two dice: $Y$ the first, $Z$ the second, $X=Y+Z$. The rows of the table are the events $\{Y=y\}$.
  \bigskip
  ...
  \begin{tabular}{@{}r@{\ \ }>{\raggedright\arraybackslash}p{4.2cm}>{\raggedright\arraybackslash}p{8.6cm}@{}}
    ...
    3. & \textbf{Given the variable:} $\E[X\mid Y]$ & Write each row's average into every cell of that row. The result is a random variable. \\
    4. & \textbf{Properties} & Tower, taking out what is known, independence. Each is a statement about the rows of the table. \\
    5. & \textbf{Two applications} & Random sums, and waiting times. Both are computed by conditioning on the first stage. \\
  \end{tabular}
\end{frame}
...
\begin{frame}{Three Problems We Cannot Yet Do}
  ...
  \begin{block}{Problem A: a random number of coins}
    ...
    Via the pmf of $H$: $p_H(h)=\sum_{n=\max(h,1)}^{6}\tfrac16\binom nh2^{-n}$ for $h=0,\dots,6$, seven sums, then $\sum_h h\,p_H(h)$.
  \end{block}
  \begin{block}{Problem B: an insurer's payout}
    ...
    Via the distribution of $S$: it is a mixture over $n$ of $n$-fold convolutions of the claim-size distribution, and neither distribution is given.
  \end{block}
  \begin{block}{Problem C: waiting for HH}
    ...
    Via the pmf of $T$: $\Prob(T=n)=F_{n-1}/2^n$ with $F_1,F_2,F_3,\dots=1,1,2,3,5,\dots$ the Fibonacci numbers, then $\sum_n n\,\Prob(T=n)$.
  \end{block}
  All three are solved below by conditioning on the first stage (slides~\ref{coins}, \ref{insurer}, \ref{hh}).
\end{frame}
...
\begin{frame}{The Table}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \smallskip
      Every random variable on this $\Omega$ assigns a number to each of the $36$ cells; every event is a set of cells, with probability the number of cells over $36$.
    \end{column}
...
\begin{frame}{Conditioning on $Y=y$ Is Restricting to a Row}
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \]
      So, given $Y=4$, $X$ takes each of the values $5,6,\dots,10$ with probability $\tfrac16$, which is the distribution of the numbers in row $4$.
    \end{column}
...
\begin{frame}{Conditional Expectation Given $Y=y$: The Row Average}
  ...
  \begin{block}{Definition (discrete case)}
    ...
    \]
  \end{block}
  \words{The ordinary expected value, computed with the conditional pmf of row $y$ in place of the pmf of $X$.}
  \begin{block}{Reading the definition}
    \begin{itemize}
      \item The variable $x$ runs over the values of $X$ and is summed out; $y$ is held fixed and names the row; the result depends on $y$ alone.
      \item $g\colon S_Y\to\R$ is a function on the values of $Y$, not on $\Omega$, so it is not a random variable.
      \item For $y\notin S_Y$, $g(y)$ is undefined, since a row of probability $0$ has no conditional distribution.
      \item $\E[X\,\ind_{\{Y=y\}}]=\sum_x x\,p_{X,Y}(x,y)=\E[X\mid Y=y]\,\Prob(Y=y)$, with $\ind_{\{Y=y\}}\colon\Omega\to\{0,1\}$ the indicator of the row. This is the total of the row, not its average.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Two Dice: The Row Average as a Function of $y$}
  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \end{exampleblock}
      \begin{itemize}
        \item $g\colon\{1,\dots,6\}\to\R$, $g(y)=y+3.5$, takes six values, one per row.
        \item $g(y)-y=3.5=\E[Z]$ for every $y$, because on row $y$ the first die is fixed at $y$ and only the second die varies. Slide~\ref{constrow} develops this into a method.
      \end{itemize}
    \end{column}
...
% ---------------------------------------------------------------------------
\begin{frame}{The Continuous Case: The Row $\{Y=y\}$ Becomes a Line}
  \small
  \begin{block}{Definition (continuous case)}
    ...
    \]
    where $x$ is a value of $X$ and $y$ is the observed value of $Y$. The \textbf{conditional expectation of $X$ given $Y=y$} is the function
    \[
      g\colon S_Y\to\R,\qquad g(y)=\E[X\mid Y=y]=\int_{-\infty}^{\infty}x\,f_{X\mid Y}(x\mid y)\dd x .
    \]
  \end{block}
  \words{The row $\{Y=y\}$ is now the horizontal line at height $y$, and the function $x\mapsto f_{X,Y}(x,y)/f_Y(y)$, which integrates to $1$, plays the part of the row's conditional pmf.}
  \begin{itemize}
    ...
    \item The procedure is otherwise unchanged: fix $y$, divide $f_{X,Y}(\cdot,y)$ by $f_Y(y)$, and integrate $x$ against the result.
  \end{itemize}
\end{frame}

% ---------------------------------------------------------------------------
\begin{frame}{The Two-Stage Point: Conditioning on $Y=y$ and on $X=x$}
  \small
  \begin{columns}[T]
    \begin{column}{0.66\textwidth}
      ...
      \begin{exampleblock}{Given $Y=y$: the horizontal line at height $y$}
        $S_Y=(0,1]$, $f_{X\mid Y}(\cdot\mid y)$ is uniform on $[0,y]$, and
        $\E[X\mid Y=y]=\int_0^y x\,\tfrac1y\dd x=\tfrac y2$.
      \end{exampleblock}
      \begin{exampleblock}{Given $X=x$: the vertical line at $x$}
      ...
      \end{exampleblock}
      \begin{itemize}
        \item Given $X=x$ the density is not uniform: $f_{Y\mid X}(y\mid x)\propto\tfrac1y$ gives more weight to small $y$, so $\E[Y\mid X=x]$ lies below the midpoint $\tfrac{1+x}2$ of $[x,1]$; at $x=\tfrac12$ it is $0.7213$ against $0.75$.
      \end{itemize}
    \end{column}
...
\begin{frame}{Functions of $X$, and What Is Constant on a Row}
  \small
  \label{constrow}%
  The definition applies to any random variable, and in particular to any function of $X$: for $W\colon\Omega\to\R$ and $h\colon\R\to\R$,
  \[
  ...
  \begin{block}{Observation: on row $y$, $Y$ is the constant $y$}
    Any random variable that depends on the outcome only through $Y$ is constant on the row. Given $Y=y$, $X=Y+Z$ equals $y+Z$, and on the row only $Z$ varies.
  \end{block}
  \begin{exampleblock}{Two dice: $\E[X^2\mid Y=y]$ without listing the row}
    ...
    \]
    Computed directly: square and average the six numbers in each of the six rows.
  \end{exampleblock}
  \begin{itemize}
    \item Slide~\ref{takeout} generalises this observation as the property ``taking out what is known''.
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \begin{block}{Theorem (law of total expectation)}
        ...
        \end{align*}
      \end{block}
      \words{Average each row, then average the row averages weighted by the row probabilities; because the rows partition the table, this is the overall average.}
    \end{column}
    \begin{column}{0.40\textwidth}
      \centering
...
\begin{frame}{Total Expectation: Proof}
  ...
  \begin{itemize}
    \item On the finite table the two sums may be swapped freely; in general the swap requires $\E|X|<\infty$.
    \item The continuous case is the same argument with $f$ in place of $p$ and $\int$ in place of $\sum$:
      $\int_{S_Y}\Big(\int x\,f_{X\mid Y}(x\mid y)\dd x\Big)f_Y(y)\dd y=\iint x\,f_{X,Y}(x,y)\dd x\dd y=\int x\,f_X(x)\dd x$.
    \item The same holds for any $W\colon\Omega\to\R$ in place of $X$: $\E[W]=\sum_y\E[W\mid Y=y]\,p_Y(y)$, in particular for $W=h(X)$.
  \end{itemize}
\end{frame}
...
\begin{frame}{Total Expectation at Work (1): Problem A, a Random Number of Coins}
  \small
  \label{coins}%
  Roll a die, getting $N\colon\Omega\to\{1,\dots,6\}$. Toss $N$ fair coins, independent of the die; $H\colon\Omega\to\{0,\dots,6\}$ is the number of heads. Find $\E[H]$.
  \begin{block}{By conditioning}
    ...
    \]
  \end{block}
  \begin{block}{Via the pmf of $H$}
    Without conditioning, one first finds $p_H(h)=\sum_{n=\max(h,1)}^{6}\tfrac16\binom nh 2^{-n}$ for $h=0,\dots,6$, seven sums (for instance $p_H(0)=\tfrac{63}{384}$, $p_H(1)=\tfrac{120}{384}$), and then computes $\sum_h h\,p_H(h)$.
  \end{block}
  \begin{itemize}
    \item On the row $\{N=n\}$ the problem reduces to the mean of a binomial distribution. The distribution of $H$, a mixture of six binomials, is not needed.
  \end{itemize}
\end{frame}

% ---------------------------------------------------------------------------
\begin{frame}{Total Expectation at Work (2): The Two-Stage Point}
  \small
  $Y$ uniform on $[0,1]$; given $Y=y$, $X$ uniform on $[0,y]$. Find $\E[X]$.
  \begin{block}{By conditioning}
    $\E[X\mid Y=y]=\tfrac y2$ and $f_Y=1$ on $(0,1]$, so
    $\E[X]=\int_0^1\tfrac y2\cdot1\dd y=\tfrac14$.
  \end{block}
  \begin{block}{Via the pdf of $X$}
    Without conditioning, one first finds the pdf of $X$ by integrating out $y$: $f_X(x)=\int_x^1 f_{X,Y}(x,y)\dd y=\int_x^1\tfrac{\dd y}{y}=-\ln x$ for $0<x<1$.
    Then $\E[X]=\int_0^1 x(-\ln x)\dd x$, which is evaluated by integration by parts and equals $\tfrac14$.
  \end{block}
\end{frame}

% ---------------------------------------------------------------------------
\begin{frame}{Total Expectation at Work (3): Variance}
  \small
  \label{variance}%
  The two-stage point again: $Y$ uniform on $[0,1]$; given $Y=y$, $X$ uniform on $[0,y]$. Find $\Var(X)$.
  \begin{block}{Definition (variance)}
    For $X\colon\Omega\to\R$ with $\E[X^2]<\infty$: \quad $\Var(X)=\E\big[(X-\E[X])^2\big]=\E[X^2]-\E[X]^2$.
  \end{block}
  \words{The mean squared distance of $X$ from its mean; computing it requires $\E[X^2]$, which can be found by total expectation.}
  \begin{itemize}
    \item Here $\E[X^2\mid Y=y]=\tfrac{y^2}3$, so $\E[X^2]=\int_0^1\tfrac{y^2}3\dd y=\tfrac19$ and $\Var(X)=\tfrac19-\tfrac1{16}=\tfrac7{144}$. Neither computation uses $f_X$.
  \end{itemize}
\end{frame}

% ===========================================================================
\begin{frame}{$\E[X\mid Y]$: Every Cell of Row $y$ Takes the Value $\E[X\mid Y=y]$}
  \small
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \begin{block}{Definition}
        ...
        \end{align*}
      \end{block}
      \words{The value at $\omega$ is the average of the row that contains $\omega$.}
      \begin{itemize}
        \item $g(Y)$ is defined on the event $\{Y\in S_Y\}$, which has probability $1$; set it to $0$ elsewhere. On the two-dice $\Omega$ every outcome has positive probability, so this does not arise.
      \end{itemize}
    \end{column}
    \begin{column}{0.42\textwidth}
      \centering
...
\begin{frame}{Three Tables, Three Levels of Knowledge}
  ...
  \begin{center}
    \begin{tabular}{@{}c@{\quad}c@{\quad}c@{}}
      ...
      the outcome $\omega$ is known & only the row $Y(\omega)$ is known & neither is known
    \end{tabular}
  \end{center}
  \words{Each table is obtained from the one to its left by averaging over what is not known.}
  \begin{itemize}
...
\begin{frame}{$\E[X\mid Y]$: Extreme Cases and the Continuous Case}
  \small
  \begin{itemize}
    \item \textbf{$\E[X\mid X]=X$.} Conditioning on $X$ itself, each ``row'' $\{X=x\}$ contains only cells where $X=x$, so its average is $x$.
    \item \textbf{$\E[X\mid Y]=\E[X]$ if $Y$ is constant.} There is then a single row, the table itself, and its average is $\E[X]$.
  ...
  \begin{block}{Continuous case}
    The same definition, $\E[X\mid Y]=g(Y)$ with $g\colon S_Y\to\R$, $g(y)=\int x\,f_{X\mid Y}(x\mid y)\dd x$. The ``rows'' $\{Y=y\}$ each have probability $0$, but $g(Y)\colon\Omega\to\R$ is an ordinary random variable, namely a function of $Y$.
  \end{block}
  \begin{exampleblock}{Examples}
    Two-stage point: $g(y)=y/2$, so $\E[X\mid Y]=Y/2$. At an outcome where $Y=0.8$, $\E[X\mid Y]$ takes the value $0.4$, whatever the value of $X$. Likewise $\E[Y\mid X]=(1-X)/(-\ln X)$.\\
    Random coins: $\E[H\mid N]=N/2$, a random variable with values $\tfrac12,1,\dots,3$, each with probability $\tfrac16$.
  \end{exampleblock}
\end{frame}

% ===========================================================================
\begin{frame}{Property 1: Tower. The Table of Row Averages Has the Same Average}
  \small
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      Replacing every entry of a row by the row average does not change the row's total. So the table of row averages has the same overall average as the original:
      ...
      \end{block}
      \words{This is the law of total expectation, with the sum over $y$ written as the expectation of a function of $Y$.}
      \begin{itemize}
        \item Since $\E[X\mid Y]$ is itself a random variable, it can in turn be conditioned on another variable.
      \end{itemize}
    \end{column}
    \begin{column}{0.44\textwidth}
      \centering
...
\begin{frame}{Property 1: Why the Tower Matters}
  \small
  To compute $\E[X]$, compute $\E[X\mid Y]$ for a suitable $Y$ and take its expectation.
  \begin{center}
  ...
  \end{center}
  \begin{itemize}
    \item In each case the distribution of $X$ is laborious to find, while $\E[X\mid Y]$ is a simple function of $Y$.
    \item The method depends on the choice of $Y$: on each row the problem must be one that can be solved directly, and $p_Y$ or $f_Y$ must be known.
    \item The last two rows are the applications at the end of the lecture.
  \end{itemize}
\end{frame}

% ---------------------------------------------------------------------------
\begin{frame}{Property 2: A Problem. The Covariance of $N$ and $H$}
  \small
  Problem A again: $N$ the die, $H$ the number of heads among $N$ coins. More coins should mean more heads on average; the covariance measures the strength of this dependence.
  ...
  \begin{block}{Direct computation}
    $\E[NH]=\sum_{n=1}^6\sum_{h=0}^n n\,h\,p_{N,H}(n,h)$ with $p_{N,H}(n,h)=\tfrac16\binom nh2^{-n}$, a sum over the $21$ nonzero terms of the joint pmf.
  \end{block}
  \begin{block}{By conditioning}
    By the tower property, $\E[NH]=\E\big[\E[NH\mid N]\big]$. On the row $\{N=n\}$ the factor $N$ equals the number $n$. If a constant factor can be taken out of a row average, then
    $\E[NH\mid N=n]=n\,\E[H\mid N=n]=n\cdot\tfrac n2$,
    and $\E[NH]$ becomes the expectation of a function of $N$. Slide~\ref{constrow} used this one row at a time; the next slide states it for all rows at once.
  \end{block}
\end{frame}
...
\begin{frame}{Property 2: Taking Out What Is Known}
  ...
  \begin{block}{Proof (discrete case)}
    ...
    This holds for every $y$; substitute $y=Y(\omega)$. The continuous case is the same, with $f$ for $p$ and $\int$ for $\sum$. \hfill$\square$
  \end{block}
  \words{Whatever is already determined by the row is a constant on the row and can be taken out of the row average.}
  \begin{block}{Linearity}
    $\E[aX+bX'\mid Y]=a\,\E[X\mid Y]+b\,\E[X'\mid Y]$ for constants $a,b$, because expectation is linear on each row.
    Combined with the theorem, $\E[h(Y)+X\mid Y]=h(Y)+\E[X\mid Y]$.
  \end{block}
\end{frame}
...
\begin{frame}{Property 2: Back to $\Cov(N,H)$, and $\E[XY]$ for Two Dice}
  ...
  \end{exampleblock}
  \begin{itemize}
    \item Both computations follow the same three steps: the tower property, taking out the factor that is a function of the conditioning variable, and an ordinary expectation of a function of that variable.
    \item The $21$-term double sum of the previous slide is not needed.
  \end{itemize}
\end{frame}
...
\begin{frame}{Property 3: Independence}
  ...
  \begin{columns}[T]
    \begin{column}{0.64\textwidth}
      ...
      \end{block}
      \words{Knowing the row gives no information about the column.}
      \begin{itemize}
    ...
    \begin{column}{0.34\textwidth}
      \centering
      $Z(\omega)=z$, the same in every row\par\medskip
      \dicetable[0.4]{0}{Z}
    \end{column}
  \end{columns}
...
\begin{frame}{Property 3: A Problem. A Random Walk, Seen From Halfway}
  \small
  \label{rw}%
  Let $\xi_1,\xi_2,\dots\colon\Omega\to\{-1,1\}$ be independent with $\Prob(\xi_i=1)=\Prob(\xi_i=-1)=\tfrac12$, and $S_n=\xi_1+\dots+\xi_n$ the position after $n$ steps. Suppose the first five steps have been observed, so that $S_5$ is known. Find $\E[S_8\mid S_5]$ and $\E[S_8^2\mid S_5]$.
  \begin{block}{Direct computation}
    There are six rows $\{S_5=s\}$, $s\in\{-5,-3,-1,1,3,5\}$. On row $s$, $S_8$ has the conditional pmf $p_{S_8\mid S_5}(t\mid s)=p_{S_5,S_8}(s,t)/p_{S_5}(s)$, which requires the joint pmf of $(S_5,S_8)$, obtained by counting paths.
    For $s=3$, $S_8$ takes the values $t=0,2,4,6$ with probabilities $\tfrac18,\tfrac38,\tfrac38,\tfrac18$, so the row has mean $3$ and second moment $12$. The other five rows are treated in the same way.
  \end{block}
  \begin{block}{By conditioning}
    Write $S_8=S_5+D$ with $D=\xi_6+\xi_7+\xi_8$. On the row $\{S_5=s\}$ the first term is the constant $s$ and can be taken out. The second term is a function of steps that the row does not involve, so restricting to the row should not change its distribution, and $\E[D\mid S_5]$ should equal $\E[D]=0$.
  \end{block}
  \begin{itemize}
    \item The statement that the row does not involve $D$ is the independence $D\indep S_5$, which holds by the fact on the previous slide. The next slide states its consequence for conditional expectations.
  \end{itemize}
\end{frame}

% ---------------------------------------------------------------------------
\begin{frame}{Property 3: Independence. Every Row Has the Distribution of the Table}
  \small
  \begin{block}{Theorem}
  ...
  \begin{block}{Proof (discrete case)}
    ...
    \]
    each row carries the same distribution of $X$ as the table itself. Hence every row average is $\sum_x x\,p_X(x)=\E[X]$.
    The continuous case is the same, using $f_{X,Y}=f_Xf_Y$. \hfill$\square$
  \end{block}
  \words{Knowing the row gives no information about $X$, so the row average is the overall average.}
  \begin{itemize}
...
% ---------------------------------------------------------------------------
\begin{frame}{Property 3: The Random Walk Done, and a Method}
  \small
  \label{rwdone}%
  ...
  \end{block}
  \begin{itemize}
    \item Two dice: $X=Y+Z$ with $Z\indep Y$, so $\E[X\mid Y]=Y+\E[Z]=Y+3.5$; and $\E[X^2\mid Y]=Y^2+2Y\,\E[Z]+\E[Z^2]$.
    \item $\E[S_8\mid S_5]=S_5$: the expected future position of the walk is its present position. This is the martingale property, taken up in the stochastic-processes lectures.
  \end{itemize}
\end{frame}
...
\begin{frame}{Property 3: What Independence Does Not Say}
  \small
  \begin{block}{The converse fails: $\E[X\mid Y]=\E[X]$ does not imply $X\indep Y$}
  ...
  \end{block}
  \begin{itemize}
    \item Equal row \emph{averages} are a weaker condition than equal row \emph{distributions}. Independence is the statement that the row distributions are equal; the theorem uses only equality of the row averages, and $\E[X\mid Y]=\E[X]$ asserts only that.
    \item $\E[X\mid Y]=\E[X]$ does imply, by the tower property and taking out what is known, that $\E[h(Y)X]=\E\big[h(Y)\,\E[X\mid Y]\big]=\E[h(Y)]\,\E[X]$, that is, $\Cov(h(Y),X)=0$ for every $h\colon\R\to\R$. In the example, $\Cov(W,Y)=\E[Y^3]-\E[W]\E[Y]=0$.
  \end{itemize}
\end{frame}
...
\begin{frame}{Properties: Summary}
  ...
  \begin{center}
    ...
    \begin{tabular}{@{}l>{$}l<{$}p{4.1cm}@{}}
      \toprule
      property & \text{statement} & reason \\
      \midrule
      tower & \E[\E[X\mid Y]]=\E[X] & replacing each row by its average keeps the total \\
      ...
      known variables & \E[h(Y)\mid Y]=h(Y),\ \ \E[X\mid X]=X & the average of a constant is that constant \\
  ...
  \begin{center}
    ...
    \begin{tabular}{@{}lp{5.6cm}p{5cm}@{}}
      ...
      object & type & meaning \\
      \midrule
      $\E[X]$ & number & average over the table \\
...
\begin{frame}{Application 1: Random Sums. Problem B}
  ...
  \]
  \words{There are two sources of randomness, the number of terms and the values of the terms.}
  \begin{block}{Problem B, via the distribution of $S$}
    $N$ claims, $\E[N]=20$; claim $i$ has size $X_i$ with $\E[X_i]=\pounds1500$. Find $\E[S]$.
    The pmf or pdf of $S$ is $\sum_n\Prob(N=n)\cdot(\text{distribution of }X_1+\dots+X_n)$, involving an $n$-fold convolution for each $n$; and here neither the distribution of $N$ nor that of the $X_i$ is specified, only the two means.
  \end{block}
  \begin{block}{By conditioning}
  ...
  \end{block}
  \begin{itemize}
    \item This proviso is an assumption about how $N$ is determined. It is stated on the next slide.
  \end{itemize}
\end{frame}
...
\begin{frame}{Application 1: Random Sums. The Assumption}
  ...
  \begin{block}{Assumption: $N$ is independent of the $X_i$}
    ...
    \]
  \end{block}
  \words{Given $N=n$, the joint distribution of $(X_1,\dots,X_n)$ is the same as the unconditional one.}
  \begin{itemize}
    \item It is not assumed that the $X_i$ are independent of each other or identically distributed, only that they have the common mean $\mu$.
    \item The assumption holds when $N$ is determined by a mechanism separate from the $X_i$: the number of customers is fixed before any of them spends, or the number of claims does not depend on their sizes.
    \item It fails when $N$ is determined by the values of the $X_i$, as in ``stop at the first six'' or ``include the second term only if it is small'' (slide~\ref{insurer}).
  \end{itemize}
\end{frame}
...
\begin{frame}{Application 1: Random Sums. Theorem and Proof}
  ...
  \end{block}
  \begin{block}{Proof}
    On the row $\{N=n\}$, $S=W_n:=X_1+\dots+X_n$. By the assumption, the row $\{N=n\}$ does not change the distribution of $W_n$, so Property 3, applied to $W_n$ on the row $\{N=n\}$, gives
    \[
  ...
  \end{block}
  \words{On the row $\{N=n\}$ the number of terms is fixed, and the mean of the sum follows by linearity.}
  \begin{itemize}
    \item The argument has the same three steps as before: condition on $N$, compute the row average by linearity, and apply the tower property.
    \item Only the mean of $S$ is computed, not its distribution.
  \end{itemize}
\end{frame}
...
\begin{frame}{Application 1: Back to the Insurer, and When the Assumption Fails}
  ...
  \end{exampleblock}
  \begin{exampleblock}{The formula fails when $N$ depends on a term}
    ...
    \]
    On the row $\{N=2\}=\{X_2\ne6\}$ the second term averages $3$, not $\mu=3.5$; restricting to the row changes the distribution of a term.
  \end{exampleblock}
  \begin{itemize}
    \item ``Stop at the first six'' also violates the assumption, yet $\mu\,\E[N]=3.5\cdot6=21$ is correct. This is Wald's identity, a separate theorem, whose hypothesis is that the decision to stop after $n$ terms depends only on the first $n$ terms. In the example above, whether $N=1$ is decided by the value of $X_2$, a later term.
  \end{itemize}
\end{frame}
...
\begin{frame}{Application 2: Waiting for HH. Problem C}
  \small
  Toss a fair coin repeatedly. Let $T\colon\Omega\to\{2,3,\dots\}$ be the number of the toss on which two consecutive heads are first completed: for T\,H\,T\,H\,H\,\dots, $T=5$. Find $\E[T]$.
  \begin{block}{Via the pmf of $T$}
    $\Prob(T=n)$ is the number of H/T strings of length $n$ that end in HH and contain no earlier HH, over $2^n$. The count is the Fibonacci number $F_{n-1}$, so $\Prob(T=n)=F_{n-1}/2^n$, and the sum $\E[T]=\sum_{n\ge2}nF_{n-1}/2^n$ is evaluated with generating functions.
  \end{block}
  Here the rows are described by events rather than by the values of a named variable, which requires the following definition.
  \begin{block}{Definition (conditional expectation given an event)}
    ...
    \]
  \end{block}
  \words{A row may be any event $A$ of positive probability; restrict to $A$, rescale, and average.}
\end{frame}
...
\begin{frame}{Application 2: Waiting for HH. The Restart Property}
  ...
  \end{block}
  \begin{block}{Why}
    $T'$ is a function of tosses $k+1,k+2,\dots$, and $\ind_{A_k}$ of tosses $1,\dots,k$: disjoint groups of independent tosses, so $T'\indep\ind_{A_k}$ and, by Property 3, $\E[T'\mid A_k]=\E[T']$. The tosses $k+1,k+2,\dots$ form a sequence of fair coin tosses, which is the experiment defining $T$, so $T'$ has the distribution of $T$ and $\E[T']=\E[T]$. \hfill$\square$
  \end{block}
  \words{On such a row, $T$ is $k$ plus an independent copy of $T$, so the row average is $k$ plus the unknown mean.}
\end{frame}
...
\begin{frame}{Application 2: Waiting for HH. The Computation}
  ...
  \begin{columns}[T]
    \begin{column}{0.6\textwidth}
      ...
      The three cases are disjoint and cover $\Omega$, so $Y$ is defined at every outcome and there are three rows.
      \medskip
    ...
    \begin{column}{0.38\textwidth}
      ...
      \end{tikzpicture}
      \par\medskip
      {\footnotesize Two of the three leaves lead back to the start, which is why $\E[T]$ appears on both sides of the equation.}
    \end{column}
  \end{columns}
\end{frame}

% ---------------------------------------------------------------------------
\begin{frame}{Application 2: Further Waiting Times}
  \small
  When a process restarts after certain outcomes, conditioning on the first step yields an equation in which the unknown mean appears on both sides, and solving it determines the mean.
  \begin{itemize}
    ...
    \item \textbf{Pattern HT}, $T_{\mathrm{HT}}$: after the first H, wait for a T. \ $\E[T_{\mathrm{HT}}]=\tfrac12\big(1+\E[T_{\mathrm{HT}}]\big)+\tfrac12\big(1+\E[T_{\mathrm T}]\big)$, so $\E[T_{\mathrm{HT}}]=4<6$. When an attempt at HT fails (with HH), the final H begins a new attempt; when an attempt at HH fails (with HT), no progress is kept.
    \item \textbf{Pattern HHH:} $14$. \ \textbf{Two sixes in a row:} $42$. The equations have the same form, with more rows.
    \item \textbf{Conditioning on three failures:} $\E[T_6\mid T_6>3]=3+\E[T_6]=9$, by the restart property at $k=3$.
  \end{itemize}
  \begin{block}{Finiteness of $\E[T]$}
