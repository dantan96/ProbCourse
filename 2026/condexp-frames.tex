...
\begin{frame}{The Continuous Case: Rows Become Slices}
  ...
  \begin{block}{Definition (continuous case)}
    ...
    \]
    where $x$ is a value of $X$ and $y$ names the slice. The \textbf{conditional expectation of $X$ given $Y=y$} is the function
    \[
  ...
  \begin{itemize}
    ...
    \item The procedure is otherwise unchanged: fix $y$, take the slice, rescale it to area $1$, and average.
  \end{itemize}
\end{frame}
...
\begin{frame}{The Two-Stage Point: Horizontal and Vertical Slices}
  ...
  \begin{columns}[T]
    \begin{column}{0.66\textwidth}
      ...
      \end{exampleblock}
      \begin{itemize}
        \item The vertical slice is not uniform: $f_{Y\mid X}(y\mid x)\propto\tfrac1y$ gives more weight to small $y$, so $\E[Y\mid X=x]$ lies below the midpoint $\tfrac{1+x}2$ of the slice; at $x=\tfrac12$ it is $0.7213$ against $0.75$.
      \end{itemize}
    \end{column}
...
\begin{frame}{Three Tables, Three Levels of Knowledge}
  ...
  \begin{center}
    \begin{tabular}{@{}c@{\quad}c@{\quad}c@{}}
      ...
      the outcome is known & only the row is known & nothing is known
    \end{tabular}
  \end{center}
...
\begin{frame}{Application 2: Waiting for HH. The Computation}
  ...
  \begin{columns}[T]
    \begin{column}{0.6\textwidth}
      ...
      Every outcome falls in exactly one case, so there are three rows.
      \medskip
