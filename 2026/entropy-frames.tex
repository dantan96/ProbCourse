...
\begin{frame}{Entropy}
  ...
  \begin{tabular}{@{}r@{\ \ }>{\raggedright\arraybackslash}p{4.4cm}>{\raggedright\arraybackslash}p{8.6cm}@{}}
    1. & \textbf{Codes} & Prefix-free codes are binary trees; a codeword of length $\ell$ excludes the fraction $2^{-\ell}$ of all strings (Kraft). \\
    2. & \textbf{Information} $-\log p$ & The ideal codeword length: the number $k$ with $2^{-k}=p$. \\
    3. & \textbf{Entropy} $H$ & Expected information; in the bar chart, the total area of bars of width $p_i$ and height $\log(1/p_i)$. \\
...
\begin{frame}{Motivation: A Robot on a Distant Moon}
  ...
  \begin{block}{Questions we cannot yet answer}
    \begin{enumerate}
      \item Can a different encoding average fewer than $2$ bits per instruction?
...
\begin{frame}{The Setting: A Source of Symbols}
  ...
  \begin{block}{Definition (source)}
    ...
    \]
  \end{block}
  \words{A source is a sequence of independent random variables with one common pmf, like repeated rolls of one $n$-sided die.}
  \begin{itemize}
...
\begin{frame}{Prefix-Free Codes Are Binary Trees}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      Read a codeword as a path from the root: $0$ = left, $1$ = right. Then $c(x)$ is a prefix of $c(x')$ if and only if the node of $c(x)$ lies on the path to $c(x')$; so a code is prefix-free if and only if \textbf{every codeword is a leaf}.
      \medskip

      Choosing $0$ as a codeword excludes every string beginning with $0$, half of all strings of every length. Choosing $10$ excludes a quarter; $110$ and $111$ an eighth each:
      \[
        \tfrac12+\tfrac14+\tfrac18+\tfrac18=1 .
      \]
      \words{A codeword of length $\ell$ excludes the fraction $2^{-\ell}$ of all strings, and in the rover's code these fractions coincide with the probabilities.}
      \medskip
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{tikzpicture}[x=0.86cm,y=0.85cm,font=\scriptsize,
        ...
        \node[soft] at (-2.2,-3.8) {excluded $\tfrac12$};
        \node[soft] at (0.4,-3.8) {excluded $\tfrac14$};
        \node[soft] at (2.6,-3.8) {excluded $\tfrac18$, $\tfrac18$};
...
\begin{frame}{Kraft's Inequality}
  ...
  \end{block}
  \words{The fractions $2^{-\ell_i}$ excluded by the codewords sum to at most $1$; and any lengths whose fractions sum to at most $1$ can be realised.}
  \begin{columns}[T]
...
Repeat for $i=1,\dots,n$. \hfill$\square$
  ...
  \begin{itemize}
    \item The converse applied to the lengths $(1,2,3,4)$: take $0$; then $00,01$ lie beneath $0$, so take $10$; then $110$; then $1110$. Excluded $\tfrac{15}{16}$; the node $1111$ stays free.
  \end{itemize}
\end{frame}
...
\begin{frame}{Information}
  \small
  In the rover's code, \up\ costs $1$ bit, \down\ $2$, \lft\ and \rgt\ $3$; and $\tfrac12=2^{-1}$, $\tfrac14=2^{-2}$, $\tfrac18=2^{-3}$. In each case the probability equals $2^{-\ell}$, where $\ell$ is the codeword length.
  \begin{block}{Definition}
  ...
  \end{block}
  \words{$I(A)$ is the number $k$ with $2^{-k}=\Prob(A)$, which is the number of fair coin tosses for which one specified sequence of outcomes has probability $\Prob(A)$.}
...
% ---------------------------------------------------------------------------
\begin{frame}{Back to the Rover: The Coded Stream Has the Distribution of Fair Coin Tosses}
  \small
  Send instructions with the code $0,10,110,111$ and look at the stream of bits, one at a time.
  \begin{block}{Each bit is a fair coin toss}
    \begin{align*}
  ...
  \end{block}
  \words{An optimally coded stream has the distribution of a sequence of fair coin tosses, and such a sequence cannot be coded in fewer than one bit per toss.}
  \begin{itemize}
    \item Applying a second compression to the rover's coded stream gains nothing: the stream is a sequence of independent fair bits, whose entropy is one bit per bit.
  \end{itemize}
...
\begin{frame}{Cross-Entropy: Two Coins}
  \small
  \begin{exampleblock}{A uniform code on a non-uniform truth}
  ...
  \end{exampleblock}
  \begin{exampleblock}{A non-uniform code on a uniform truth}
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \words{The uniform code wastes half a bit on the non-uniform truth; the non-uniform code wastes three quarters of a bit on the uniform truth, since its $3.3$-bit codeword is used with probability $\tfrac12$.}
...
\begin{frame}{Relative Entropy ($D$)}
  ...
  \begin{exampleblock}{The rover}
    ...
    Two terms are negative and two positive; the positive ones belong to the more probable symbols, so the total is positive.
  \end{exampleblock}
  \begin{itemize}
  ...
  \begin{center}
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{2.3cm}>{\raggedright\arraybackslash}p{4.2cm}>{\raggedright\arraybackslash}p{6.6cm}@{}}
      ...
      symmetry & $P=(\tfrac12,\tfrac12)$, $Q=(\tfrac14,\tfrac34)$ &
        $D(P\|Q)=1.2075-1=0.2075$: the $2$-bit codeword is used with probability $\tfrac12$ instead of $\tfrac14$. \newline
...
\begin{frame}{Why the Loss Is a Logarithm}
  ...
  \begin{block}{Proof}
    ...
    For $s,t>0$ with $s+t<1$ use $p=\big(s,t,\tfrac{1-s-t}{n-2},\dots,\tfrac{1-s-t}{n-2}\big)$ (possible as $n\ge3$): $s\,f'(s)=t\,f'(t)$. For $s+t\ge1$ compare both with a small $u$: $s\,f'(s)=u\,f'(u)=t\,f'(t)$ once $s+u<1$ and $t+u<1$.
...
\begin{frame}{What Goes Wrong With Any Other Penalty}
  ...
  \begin{exampleblock}{Linear penalty $f(t)=1-t$}
    Expected penalty $1-\sum_yp_yq_y$, smallest when $q$ puts probability $1$ on the most probable class: for $p=(0.5,0.3,0.2)$ the minimiser is $q=(1,0,0)$, penalty $0.5$, while reporting $q=p$ costs $0.62$. The penalty is smallest when the probable class is overstated.
  \end{exampleblock}
...
\begin{frame}{Motivation: Which Wordle Opener?}
  ...
  \begin{block}{Uncertainty before the first guess}
    Before the first guess the answer is uniform on $2315$ words, an uncertainty of $\log2315=11.18$ bits. A natural measure of a guess is the amount by which it reduces this, but the reduction depends on which of the $243$ patterns is returned.
  \end{block}
  \begin{block}{Questions we cannot yet answer}
    \begin{enumerate}
...
\begin{frame}{A Guess Is a Partition of the Remaining Answers}
  ...
  \begin{itemize}
    \item Seeing pattern $\pi$ shrinks the candidate list from $N$ to $|\text{bucket of }\pi|=N\,p_\Pi(\pi)$. The information of the pattern is $I_\Pi(\pi)=\log\frac1{p_\Pi(\pi)}=\log\frac{N}{|\text{bucket}|}$: the logarithm of the factor by which the list shrinks.
...
\begin{frame}{The Entropy of a Guess Is Its Expected Information}
  \small
  \begin{block}{Definition}
    The \textbf{entropy of the guess} $g$ is the entropy of its pattern, $H(\Pi)=\sum_\pi p_\Pi(\pi)\log\frac1{p_\Pi(\pi)}=\E[I_\Pi(\Pi)]$, the expected logarithm of the factor by which the list shrinks.
  \end{block}
  ...
  \begin{exampleblock}{Toy Wordle}
    \begin{tabular}{@{}lll@{}}
      ...
      \textsf{AM} & $5,1$ & $\tfrac56\cdot0.263+\tfrac16\cdot2.585=0.650$ bits; $0.263$ bits with probability $\tfrac56$ \\
      \bottomrule
    \end{tabular}
  \end{exampleblock}
...
% ---------------------------------------------------------------------------
\begin{frame}{Back to the Openers: $\Prob(G\le2)\le0.105$ for Every Strategy}
  \small
  Let $G\colon\Omega\to\{1,2,\dots\}$ be the number of guesses a strategy takes to reach the answer; the score of the strategy is $\E[G]$.
  \begin{block}{Claim}
...
\begin{frame}{Entropy Rate of a Markov Model}
  \small
  \begin{block}{Definition}
    A pmf $\pi$ on $S$ is \textbf{stationary} for $Q$ if $\sum_x\pi(x)Q(x,x')=\pi(x')$ for all $x'$, so that $X_i$ has pmf $\pi$ for every $i$. For the Markov model started from $\pi$, the \textbf{entropy rate} is
    \[
...
% ---------------------------------------------------------------------------
\begin{frame}{Entropy Rate: The Entropy of $n$ Letters}
  \small
  \begin{block}{Fact}
    For the Markov model started from $\pi$, $n$ letters have entropy $H(X_1,\dots,X_n)=H(\pi)+(n-1)H(X_{i+1}\mid X_i)$, by the chain rule for $n$ variables (by induction) and the Markov property $H(X_{k+1}\mid X_1,\dots,X_k)=H(X_{k+1}\mid X_k)$: each letter depends on the past only through the last letter (stated without proof).
  \end{block}
  \words{The first letter costs $H(\pi)$ bits and each later letter costs the entropy rate, so per letter the entropy of $n$ letters tends to the entropy rate.}
  \begin{itemize}
    \item The entropy rate is therefore the lower bound for compressing the model's text (Section 4 applied to blocks of $n$ letters).
  \end{itemize}
\end{frame}
